...
system = z2pack.fp.System(
    ...
    command="mpirun -np 7 ~/software/abinit-7.10.5/src/98_main/abinit < {}_nscf.files >& log".format(
        SYSTEM_NAME
    ),
    executable="/bin/bash",
)
...
res_0 = z2pack.surface.run(
    system=system, surface=lambda s, t: [0, s / 2, t], save_file="res_0.msgpack"
)
res_1 = z2pack.surface.run(
    system=system, surface=lambda s, t: [0.5, s / 2, t], save_file="res_1.msgpack"
)
...
print(f"Z2 topological invariant at kx = 0: {z2pack.invariant.z2(res_0)}")
print(f"Z2 topological invariant at kx = 0.5: {z2pack.invariant.z2(res_1)}")
