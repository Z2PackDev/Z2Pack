...
snte_plus = z2pack.fp.System(
    input_files=[os.path.join("input", fn) for fn in ["snte.nscf.in", "snte.pw2z2+i.in"]],
...
res_plus = z2pack.surface.run(
    system=snte_plus, surface=surface, save_file="results/res_plus.json", load=True, **settings
)

res_minus = z2pack.surface.run(
    system=snte_minus, surface=surface, save_file="results/res_minus.json", load=True, **settings
)
