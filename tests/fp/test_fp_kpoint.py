...
def kpt(line):
    ...
    """
    kpt = [np.array(line(tval)) for tval in [0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0]]
    return kpt


VALS = [0.0, 0.25, 0.5, 0.75, 1.0]

STRAIGHT_SIMPLE = [lambda t, s1=s1, s2=s2: [s1, s2, t] for s1 in VALS for s2 in VALS]
STRAIGHT_SIMPLE.extend([lambda t, s1=s1, s2=s2: [s1, t, s2] for s1 in VALS for s2 in VALS])
STRAIGHT_SIMPLE.extend([lambda t, s1=s1, s2=s2: [t, s1, s2] for s1 in VALS for s2 in VALS])
...
VALID_COMPARABLE = (
    STRAIGHT_SIMPLE + STRAIGHT_ANY_DIRECTION + STRAIGHT_UNEQUAL_SPACING + STRAIGHT_MULTIPLE_BZ
)
...
VALID_LINES = {
    z2pack.fp.kpoint.vasp.__name__: {
        "fct": z2pack.fp.kpoint.vasp,
        "valid_comparable": STRAIGHT_SIMPLE,
        "valid_incomparable": [],
        "invalid": STRAIGHT_ANY_DIRECTION
    ...
    z2pack.fp.kpoint.abinit.__name__: {
        ...
        "valid_comparable": STRAIGHT_SIMPLE + STRAIGHT_ANY_DIRECTION + STRAIGHT_MULTIPLE_BZ,
        "valid_incomparable": [],
