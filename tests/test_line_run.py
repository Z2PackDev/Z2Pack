...
def test_tb_convention(pos_tol, tb_system, tb_model, tb_line):
    ...
    result2 = z2pack.line.run(system=tb_system_convention1, line=tb_line, pos_tol=pos_tol)
...
def test_iterator_3(simple_system, simple_line, compare_equal):
    ...
    """
    result = z2pack.line.run(
        system=simple_system, line=simple_line, iterator=[4, 12, 21], pos_tol=None
    )
...
def test_invalid_restart(simple_system, simple_line):
    ...
    with pytest.raises(ValueError):
        z2pack.line.run(system=simple_system, line=simple_line, init_result=result, load=True)
...
def test_file_restart(simple_system, simple_line):
    ...
    with tempfile.NamedTemporaryFile() as temp_file:
        result1 = z2pack.line.run(system=simple_system, line=simple_line, save_file=temp_file.name)
